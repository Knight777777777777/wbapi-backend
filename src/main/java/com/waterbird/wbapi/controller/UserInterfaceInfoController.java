...
import com.waterbird.wbapi.model.dto.userinterfaceinfo.UserInterfaceInfoAddRequest;
import com.waterbird.wbapi.model.dto.userinterfaceinfo.UserInterfaceInfoQueryRequest;
import com.waterbird.wbapi.model.dto.userinterfaceinfo.UserInterfaceInfoUpdateRequest;
...
public class UserInterfaceInfoController {
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> deleteUserInterfaceInfos(@RequestBody DeleteBatchRequest deleteBatchRequest, HttpServletRequest request) {
        ...
        if (userInterfaceInfos.isEmpty()) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR);
        }

        for (UserInterfaceInfo userInterfaceInfo : userInterfaceInfos) {
            // 检查权限
            if (!userInterfaceInfo.getUserId().equals(user.getId()) && !userService.isAdmin(user)) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
            }
        }

        boolean success = userInterfaceInfoService.removeByIds(ids);
